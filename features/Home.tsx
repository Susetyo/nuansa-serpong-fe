        ...
        <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-card to-charcoal-light p-6 shadow-card">
          <div className="flex items-start justify-between">
            <div>
              ...
              {/* Tier Badge */}
              <div
                className={`mt-2 inline-flex items-center gap-1 rounded-full bg-gradient-to-r px-3 py-1 text-xs font-semibold text-black ${
                  tierColors[displayUser.tier]
                }`}
              >
            ...
            <div className="rounded-lg bg-white p-2">
              <QRCode value={displayUser.memberNumber} size={72} level="M" />
            </div>
          </div>
        </div>
          ...
          {points.expiryBuckets.length > 0 && (
            <div className="mt-3 rounded-lg bg-destructive/10 px-3 py-2">
              <p className="text-xs text-destructive">
                ⚠️ {points.expiryBuckets[0].points.toLocaleString()} points akan
                kadaluarsa {points.expiryBuckets[0].expiryDate}
              </p>
            </div>
          )}
        </div>
